...
def _get_model_class(model_type: str):
    ...
    if model_type == 'xgboost':
        ...
        raise ValueError(f"model_type inconnu: {model_type}. "
                        f"Valeurs acceptees: xgboost, lightgbm, catboost, randomforest")
...
def _convert_params(params: Dict, model_type: str) -> Dict:
    ...
    for key, value in params.items():
        # Parametres entiers
        int_params = {
            'n_estimators', 'max_depth', 'min_child_weight', 'num_leaves',
            'min_child_samples', 'iterations', 'depth', 'min_samples_split',
            'min_samples_leaf'
        }
...
def optimize_boosting_model(
    ...
    if model_type not in DEFAULT_SEARCH_SPACES:
        raise ValueError(f"model_type '{model_type}' non supporte. "
                        f"Valeurs acceptees: {list(DEFAULT_SEARCH_SPACES.keys())}")
...
def plot_optimization_history(trials: Trials, scoring: str = 'score') -> None:
    ...
    fig.update_layout(
        title='Convergence Hyperopt',
        xaxis_title='Numero essai',
